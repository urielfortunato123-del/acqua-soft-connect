    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      >
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div className="space-y-2">
              <h2 className="text-2xl font-extrabold text-primary">Última Manutenção</h2>
              <p className="text-muted-foreground font-medium">Quando foi a última revisão completa?</p>
            </div>
            <div className="bg-card p-6 rounded-[2rem] shadow-sm border border-border grid grid-cols-1 gap-3">
              {["Menos de 6 meses", "1 ano", "Mais de 1 ano", "Não sei informar"].map(opt => (
                <button 
                  ...
                  className={`w-full p-4 rounded-2xl font-bold text-left transition-all ${watchAll.ultima_manutencao === opt ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}
                >
                  {opt}
                </button>
              ))}
        ...
        {step === 4 && (
          <div className="space-y-6">
            <div className="space-y-2">
              <h2 className="text-2xl font-extrabold text-primary">Status do Equipamento</h2>
              <p className="text-muted-foreground font-medium">O equipamento está funcionando normalmente?</p>
            </div>
            <div className="bg-card p-6 rounded-[2rem] shadow-sm border border-border grid grid-cols-2 gap-4">
              {["Sim", "Não"].map(opt => (
                <button 
                  ...
                  className={`w-full p-6 rounded-[2rem] font-bold transition-all flex flex-col items-center gap-3 ${watchAll.funcionando === opt ? 'bg-primary text-primary-foreground shadow-lg' : 'bg-muted text-muted-foreground'}`}
                >
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${watchAll.funcionando === opt ? 'bg-white/20' : 'bg-muted-foreground/10'}`}>
        ...
        {step === 5 && (
          <div className="space-y-6">
            <div className="space-y-2">
              <h2 className="text-2xl font-extrabold text-primary">Observações Adicionais</h2>
              <p className="text-muted-foreground font-medium">Algo mais que gostaria de nos contar?</p>
            </div>
        ...
        {step === 6 && (
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground font-medium">Opcional: uma foto do seu purificador.</p>
            </div>
            <div className="bg-card p-6 rounded-[2rem] shadow-sm border border-border space-y-4">
